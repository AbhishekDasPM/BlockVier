...
describe("LazyNFT", function () {
  ...
  it("Should deploy", async function () {
    ...
    const LazyNFT = await ethers.getContractFactory("LazyNFT");
     lazynft = await LazyNFT.deploy(minter);
    await lazynft.deployed();

    console.log(`LazyNFT deployed at ${lazynft.address}`);
    const Market = await ethers.getContractFactory("Market");
    market = await Market.deploy(lazynft.address);
  ...
  it("should by a nft voucher", async () => {
    const { contract, redeemerContract, redeemer, minter } = await deploy();
    ...
    const lazyMinter = new LazyMinter({ contract: market, signer: minter });
    const voucher = await lazyMinter.createVoucher(
      1,
      "ipfs://bafybeigdyrzt5sfp7udm7hu76uh7y26nf3efuylqabf3oclgtqy55fbzdi"
    );
    await market
      .connect(redeemer)
      .buyNFTVoucherNowWithEth(redeemer.address, voucher, 1, { value: 1 });
    const tx = await market.connect(minter).availableToWithdrawSeller();
